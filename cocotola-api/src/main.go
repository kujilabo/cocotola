...
func main() {
	...
	rff := func(ctx context.Context, db *gorm.DB) (appS.RepositoryFactory, error) {
		return appG.NewRepositoryFactory(ctx, db, cfg.DB.DriverName, time.UTC, jobRff, userRff, pf, problemRepositories) // nolint:wrapcheck
	}
...
func initApp1(ctx context.Context, appTransaction appS.Transaction, password string) {
	...
	addOrganizationFunc := func(ctx context.Context, systemAdmin userS.SystemAdmin) error {
		...
		if err != nil {
			return liberrors.Errorf("NewFirstOwnerAddParameter. err: %w", err)
		}
		...
		if err != nil {
			return liberrors.Errorf("AddOrganization. err: %w", err)
		}
